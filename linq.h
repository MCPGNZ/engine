...
template < template <typename T, typename... A> class Collection, typename T, typename F, typename... A,
    typename = IS_TRUE_VALUE(pk::function_traits<F>::parameter_count == 1),
    typename = IS_SAME_TYPE(pk::function_traits<F>::return_type, bool),
    typename = IS_SAME_TYPE(pk::function_traits<F>::parameter<0>, const T&)
>
decltype(auto) operator >> (const Collection<T, A...>& collection, const pk::internals::where_proxy<F>& where)
{
...
template < template <typename T, typename... A> class Collection, typename T, typename F, typename... A,
    typename = IS_TRUE_VALUE(pk::function_traits<F>::parameter_count == 1),
    typename = IS_SAME_TYPE(pk::function_traits<F>::parameter<0>, const T&),
    typename = IS_DIFF_TYPE(pk::function_traits<F>::return_type, void)
>
decltype(auto) operator >> (const Collection<T, A...>& collection, const pk::internals::select_proxy<F>& select)
{
    const auto size = collection.size();

    Collection<function_traits<F>::return_type, A...> result(size);
    for(std::remove_cv_t<decltype(size)> i = 0; i < size; i++)
    {
...
template < template <typename T, typename... A> class Collection, typename T, typename F, typename... A,
    typename = IS_TRUE_VALUE(pk::function_traits<F>::parameter_count == 1),
    typename = IS_SAME_TYPE(pk::function_traits<F>::parameter<0>, T&),
    typename = IS_SAME_TYPE(pk::function_traits<F>::return_type, void)
>
decltype(auto) operator >> (Collection<T, A...>& collection, const pk::internals::foreach_proxy<F>& foreach)
{
...
template < template <typename T, typename... A> class Collection, typename T, typename F, typename... A,
    ...
    typename = IS_SAME_TYPE(pk::function_traits<F>::return_type, bool)
>
decltype(auto) operator >> (const Collection<T, A...>& collection, const pk::internals::any_proxy<F>& any)
{
...
template < template <typename T, typename... A> class Collection, typename T, typename F, typename... A,
    ...
    typename = IS_SAME_TYPE(pk::function_traits<F>::return_type, bool)
>
decltype(auto) operator >> (const Collection<T, A...>& collection, const pk::internals::all_proxy<F>& all)
{
    for(const auto& value : collection)
    {
        if(!all.method(value))
...
>
decltype(auto) operator >> (const Collection<T, A...>& collection, const pk::internals::contains_proxy<V>& contains)
{
    for(const auto& value : collection)
    {
...
decltype(auto) operator >> (const Collection<T, A...>& collection, const pk::internals::count_proxy&)
{
...
//[min]
template < template <typename T, typename... A> class Collection, typename T, typename... A>
decltype(auto) operator >> (const Collection<T, A...>& collection, const pk::internals::min_proxy&)
{
    const auto size = collection.size();
...
//[max]
template < template <typename T, typename... A> class Collection, typename T, typename... A>
decltype(auto) operator >> (const Collection<T, A...>& collection, const pk::internals::max_proxy&)
{
    const auto size = collection.size();
...
//[sum]
template < template <typename T, typename... A> class Collection, typename T, typename... A>
decltype(auto) operator >> (const Collection<T, A...>& collection, const pk::internals::sum_proxy&)
{
    T result = 0;
    for(const auto& value : collection)
    {
